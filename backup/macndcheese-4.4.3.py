...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.install_gptk_full_btn = QPushButton("Install GPTK Full")
        self.install_gptk_full_btn.setStyleSheet("padding: 12px 24px; font-size: 16px; background-color: transparent; border: 2px solid #FFFFFF; border-radius: 16px;")
        self.install_gptk_full_btn.clicked.connect(self.install_gptk_full)
        
        steam_buttons_layout.addWidget(self.btn_install_steam)
        steam_buttons_layout.addWidget(self.btn_launch_steam)
        steam_buttons_layout.addWidget(self.install_gptk_full_btn)
    ...
    def _build_empty_state_view(self) -> None:
        ...
        layout = QVBoxLayout(self.steam_view)
        ...
        if not ok:
            ...
            if "xcode command line tools" in lower or "clt install" in lower:
                QMessageBox.warning(
                    self,
                    APP_NAME,
                    "Xcode Command Line Tools are required before setup can continue. MacNCheese will now open the installer in Terminal.",
                )
            ...
            if "need sudo access on macos" in lower:
                QMessageBox.warning(
                    self,
                    APP_NAME,
                    "MacNCheese needs sudo access on macOS to install certain components. Please enter your macOS password in the Terminal window.",
                )
            ...
            if "password was rejected" in lower:
                QMessageBox.warning(
                    self,
                    APP_NAME,
                    "The macOS password was rejected. Please ensure you are using an Administrator account.",
                )
    ...
    def scan_games(self) -> None:
        ...
        self.btn_add_container.setVisible(has_content)
        if hasattr(self, "_steam_sidebar_btn") and self._steam_sidebar_btn:
            self._steam_sidebar_btn.setVisible(has_content)
